...
public:
    Button(float x, float y, float width, float height, string buttonText)
    {
...
int main()
{
    ...
    RenderWindow window(VideoMode(imageSize.x, imageSize.y), "SFML Button Banako");

    Sprite backgroundSprite(backgroundTexture);

    Button button(imageSize.x / 2 - 100, imageSize.y / 2 - 25, 200, 50, "Start Game");
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (button.isMouseOver(window) && !gameRunning)
                {
                    runGame();
                    gameRunning = true;
                    window.close(); // Close the button window when the game starts
                }
            }
        }
